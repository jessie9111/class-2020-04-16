...
Since the income coefficient is negative, people with a higher income will be slightly more likely to vote for the Republican candidate than people with a lower income (about 4-5% more likely to vote for the Republican candidate with each increase on the five point scale).
...
nes_tree <- fit(update(tree_mod, cost_complexity = 0),
                vote ~ ideology + income + gender + race,
                data = nes)
...
nes %>% 
  ...
  d3rain(income, vote, toolTip = income, title = "Voting Based on Income Category in 2000") %>%
  drip_settings(dripSequence = 'iterate',
                ease = 'bounce',
                jitterWidth = 20,
                dripSpeed = 1000,
                dripFill = 'steelblue') %>% 
  chart_settings(fontFamily = 'calibri',yAxisTickLocation = 'left')
